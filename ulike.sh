#!/bin/bash
#抓包ulike小程序，抓smp-api.iyouke.com域名，抓Authorization参数。
#青龙创建环境变量，变量名ulike，值为Authorization，多个账号就创建多个变量
#by-莫老师，版本1.3
#cron:35 7 * * *
...
do
authorization=${zh[$s]}
qd=$(curl -s -X GET -H "Host: smp-api.iyouke.com" -H "authorization: $authorization" -H "appid: wx5c14645c58f75f89" "https://smp-api.iyouke.com/dtapi/pointsSign/user/sign?date=$(date +%Y/%m/%d)" -k)
if [ "$qd" = token已失效 ]; then
